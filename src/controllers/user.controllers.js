...
const generateAccessAndRefreshToken = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Something wentwrong while generating :: generateAccessAndRefreshToken",
    );
  }
};
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  const avatar = await uploadOnCloudinary(avatarLocalPath);
  if (!avatar.url)
    throw new ApiError(400, "Error occur while uploading in avatar");
  ...
  if (previousAvatar) await deleteFromCloudinary(previousAvatar);
  return res
    .status(200)
...
const updateCoverImage = asyncHandler(async (req, res) => {
  const previousUser = await User.findById(req._id);
  const previousCoverImage = previousUser?.coverImage;
  const coverImageLocalPath = req.file?.path;
  if (!coverImageLocalPath)
    throw new ApiError(400, "Cover Image file is missing");
  const coverImage = uploadOnCloudinary(coverImageLocalPath);
  if (!coverImage.url)
    throw new ApiError(400, "Error occur while uploading in cover Image");
  const user = await User.findByIdAndUpdate(
    req.user?._id,
    {
      $set: {
  ...
  if (previousCoverImage) await deleteFromCloudinary(previousCoverImage);
  return res
    .status(200)
